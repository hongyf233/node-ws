...
const UUID = process.env.UUID || '7b6aa69e-414d-474e-ad0e-b8bfe69059ca';
...
const GetISP = async () => {
  try {
    ...
    const data = res.data;
    ISP = `${data.country}-${data.asOrganization}`.replace(new RegExp(" ", "g"), '_');
  } catch (e) {
    ISP = 'Unknown';
  }
};
...
function resolveHost(host) {
  return new Promise((resolve, reject) => {
    if (new RegExp("^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$", "").test(host)) {
      resolve(host);
      return;
    }
    let attempts = 0;
...
function handleVlessConnection(ws, msg) {
  ...
  const host = ATYP == 1 ? msg.slice(i, i += 4).join('.') : ATYP == 2 ? new TextDecoder().decode(msg.slice(i + 1, i += 1 + msg.slice(i, i + 1).readUInt8())) : ATYP == 3 ? msg.slice(i, i += 16).reduce((s, b, i, a) => i % 2 ? s.concat(a.slice(i - 1, i + 1)) : s, []).map(b => b.readUInt16BE(0).toString(16)).join(':') : '';
...
uuid: ${UUID}`;
  ...
  try {
    exec(command, {
      ...
      if (err) console.error('npm running error:', err);else console.log('npm is running');
    });
  } catch (error) {
